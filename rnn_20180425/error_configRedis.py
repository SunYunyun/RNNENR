...
pinyin=Pinyin()
# import redis
from config_database import config,clients,redis_clients
...
def rule_error():
    stopword4Video = 'woxiang xiangkan yankan laiyi yibu youmei meiyou haoma dianying de dianyou dedian jiayi shouye shouji' \
                 ...
                 'yande zhuyan canyu zhizuo shede daode danyan paide paishe huijia xiaohuang'
    ...
    return stopword4Video,stopword4Figure,MinThresholds
...
def writeRedis():
    stopword4Video, stopword4Figure,MinThresholds=rule_error()
    videos, celebrity, hot_role = update_data()
    # pipe = conR.pipeline(transaction=True)
    pipe = redis_clients.conR.pipeline(transaction=True)
    ...
    redis_clients.redis_set('semantic:error_correct:stopword4Video', stopword4Video)
    redis_clients.redis_set('semantic:error_correct:stopword4Figure', stopword4Figure)
    redis_clients.redis_set('semantic:error_correct:MinThresholds', MinThresholds)
    redis_clients.redis_hmt('semantic:error_correct:ConfusingVideo',videos)
    redis_clients.redis_hmt('semantic:error_correct:ConfusingFigure',celebrity)
    redis_clients.redis_hmt('semantic:error_correct:ConfusingRole',hot_role)
