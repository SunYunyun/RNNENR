...
from config_database import arrango2redis
...
class Domain(object):
    ...
    def __init__(self,command):
        ...
        if '直播' in command:
            self.category='直播'
        elif ('电视剧' in command or '国产剧' in command or '日剧' in command or '电视连续剧'in command or '连续剧' in command or '韩剧' in command or '美剧' in command or '国产剧' in command\
                or '港台剧' in command or '陆剧' in command) and '电视剧台' not in command:
            self.category =  "电视剧"
        elif '看节目' in command:
            self.category = '推荐节目'
        elif ('综艺' in command or '综艺节目 ' in command) and '综艺台' not in command :  # 20180117 d
            self.category = '综艺'
        elif ('电影' in command or '影片' in command or '观影' in command or '片子' in command) and '电影台' not in command:#20180116 d
            self.category = '电影'
...
class nameOnly(object):
    ...
    def find_key_word(self):
        command_words=['我想看','我想要','我要看','来一','找一找','我想要看', '我想点播', '我想播放', '想播放', '想点播', '有没有', '想看', '点播', '播放', '想播','来部',
                        '放', '看', '点播', '播', '有什么好看的','打开','搜索','想放', '想要',  '点播','转到','调到','切换','切换到','我要点','我看','请播放','来一部',
            ...
            if len(scoreDocs) != 0:
                ...
                if 'appname' in p and '打开' in self.command:
                    ...
                    for doc in scoreDocs:
                        ...
                        if ts not in directo_actor.video_domain:
                            ts=''
                            continue
                        ...
                        if ts in ['appname','station'] :
                            ...
                            #     break #
                            tp_ = doc['profession_active']  # --add 20180117 s
                            eflag = 1  #
                            ...
                            if tp_ != None:
                                for i in tp_:
                                    ...
                                    if i['profession'] in ['sportsman', 'sportsCoach',
                                                           'singer']:  # --dd 20180117 s'writer'
                                        eflag = 0
                                        continue
                                    scoreList.append(i['score'])

                                    if i['score'] >= 0.2:
                                        final[i['profession']] = tm
                                        actorflag[i['profession']] = tm

                                if scoreList != [] and max(scoreList) < 0.2:
                                    final[tp_[scoreList.index(max(scoreList))]['profession']] = tm
                                    actorflag[i['profession']] = tm  # --dd 20180117 s
    ...
    def search_redis(self):
      ...
      redis_list = ['figure','film','role']
      for name in redis_list:
        if True == arrango2redis.redis_verify(name,self.name):
          self.final = self.name
          if name == 'figure':
            self.label = 'actor'
          elif name == 'film':
