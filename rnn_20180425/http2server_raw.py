...
import logg
import url_request
# import code_lizhuoxuan.pos_rnn as ppos
...
#conR=redis.Redis(host=config.redis_ip,port=redis_port,db=redis_db_ec)
#mongoClient=MongoClient(host='10.9.201.190',port=27017)
...
#deal non-necessary words
    ...
    def get_celebrities(self, txt,redisName):
        # stop_words =str(conR.get('semantic:error_correct:stopword4Figure'))
        stop_words = str(redis_clients.redis_get('semantic:error_correct:stopword4Figure'))
        ...
        out = set(redis_clients.redis_hmt(redisName, suggest))
        for i in out:
    ...
    def get_video(self,txt, redisDBname):
        ...
        stop_words = str(redis_clients.redis_get('semantic:error_correct:stopword4Video'))
        ...
        out = set(redis_clients.redis_hmt(redisDBname, candidate_py))
        for i in out:
            if i is not None and ',' not in i:
                out_candidate.add(i.decode('utf-8'))
            elif i is not None and ',' in i:
...
#result form construction
    def result_form(self, result,category):
        ...
        if domain != 'FailCorrecting' and domain != 'FailDetecting':
            ...
            scoreDocs_error = clients.query_formates(oralname,-1)
    ...
    def writeToMongo(self, result, mongodic):
        ...
        if category != 'FailDetecting' and category != 'FailCorrecting' and category!='Unsupported':
            ...
            if mongodic.has_key('suggestedmovies') or mongodic.has_key('suggestedcelebrities') \
                ...
                mongo_ec.ec_wrong_insert(dicMerged)
#match figure,video,role
    def singular_match(self, txt, sInput_py):
        ...
        # ScoreSet =conR.get('semantic:error_correct:MinThresholds')
        ScoreSet = redis_clients.redis_get('semantic:error_correct:MinThresholds')
        ...
        for actorname_key in suggestlist_actor:
            ...
            elif minScoreSet[0] < fuzz_ratio2 <= 95:
        ...
        for movie_name_key in suggestlist_movie:
            ...
            # print movie_name_key,fuzz_ratio1
            ...
            if fuzz_ratio1 >= 96:
                ...
                return result, mongodic
            elif minScoreSet[1]<= fuzz_ratio1 < 96:
        ...
        for role_name in suggerslit_hotrole:
            ...
            if fuzz_ratio3 >= 96:
                ...
                return result, mongodic
            elif minScoreSet[2]< fuzz_ratio3 < 96:
...
# src_txt = 'IDO'
# t = proofcheck()
# print type(t.proofreadAndSuggest(u"%s"%src_txt))
#{'category': u'\u7535\u5f71', u'actor': u'\u8d75\u8587'} #'我想看赵微的电影'
# if t.proofreadAndSuggest(u"%s"%src_txt):
#     print 1
# else:print 0
'''
RNN_LSTM
