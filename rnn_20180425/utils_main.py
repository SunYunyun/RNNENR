...
import re
from num_process import numberTranform
...
class Multi_character_star(object):
        ...
        def search(tmp, searchertv, TermQuery, Term):
            ...
            if len(scoreDocs) != 0:
                for doc in scoreDocs:
                    ...
                    ts_ = doc.get("label")
                    if ts_ not in directo_actor.video_domain:
                        ts_=''
                        continue
    ...
    def single_actor(self,searchertv,TermQuery,Term):
        ...
        if re.findall(r'[0-9]+',self.command):
            self.command = numberTranform(self.command)

        scoreDocs = clients.query_formates(self.command,8)
        ...
        if len(scoreDocs) != 0:
            ...
            for doc in scoreDocs:
                ...
                if 'profession' in doc: tp_ = doc['profession_active']  # add #20180117 s
                if ts_ in ['appname', 'station']:
                    continue
                elif ts_== 'figure' and tp_ != None:
                    tflag=1
                    scoreList = []  # ad 20180124
                    for i in tp_:
                        # if i in ['compose','producer','photographer']:
                        #     continue
                        # elif i == 'singer':
                        #     continue
                        if i['profession'] in ['sportsman','producer','sportsCoach', 'singer','writer']:#20180124
                            tflag = 0
                            continue
                        scoreList.append(i['score'])  # ad 20180124 s
                        if ('配 音' in self.origal or u'配 音' in self.origal):
                        ...
                        #         tp.pop('director')
                            ...
                            if self.command in tp.values() and i['profession'].encode('utf-8') in tp.keys():  # --add 20180117 s
                                pass
                            else:
                                if i['score'] >= 0.2:
                                    tp[i['profession'].encode('utf-8')] = ta_.encode('utf-8')  # --add 20180124 s
                    # print max(scoreList)
                    if scoreList != [] and max(scoreList) < 0.2:
                        tp_[scoreList.index(max(scoreList))]['profession'] = ta_  # --add 20180124 s
                elif ts_== 'film':
                    if 'name' in tp:
